...
function isActivated() {
    try {
        ...
        $stmt = $db->query("SELECT setting_value FROM restaurant_settings WHERE setting_key = 'hardware_id'");
        $row = $stmt->fetch();
        if ($row) {
            return HardwareID::verify($row['setting_value']);
        }
    } catch (Exception $e) {}
    return false;
}
...
$cacheFile = __DIR__ . '/../cache/categories.php';
if (file_exists($cacheFile) && (time() - filemtime($cacheFile) < 300)) {
    $categories = include $cacheFile;
} else {
    try {
        ...
        file_put_contents($cacheFile, '<?php return ' . var_export($categories, true) . ';');
    } catch (Exception $e) {
        $categories = [];
...
// --- TABLES (no caching – they change often) ---
try {
    $database = new Database();
    $db = $database->getConnection();
    ...
    <style>
        ...
        * {
        ...
        body {
            ...
            background: linear-gradient(145deg, #d9e4f5 0%, #c0d0e8 100%);
            min-height: 100vh;
            padding-bottom: 80px;
        }
        .wizard-container {
        ...
        .table-card-select.selected {
            border-color: #27ae60;
            background: linear-gradient(145deg, #e0f2e9, #c8e6d9);
        ...
        @media (max-width: 768px) {
            .wizard-container { padding: 10px; padding-bottom: 80px; }
            .items-grid-visual { grid-template-columns: repeat(2, 1fr); gap: 12px; }
            ...
            .table-card-select { padding: 12px 8px; }
            .table-number-large { font-size: 1.1rem; }
        }
...
<body>
    <div class="wizard-container">
        ...
        <div class="progress-steps">
            <div class="step" data-step="1" id="step1-indicator"><div class="step-number">1</div><div class="step-label"><?php echo t('select_table'); ?></div></div>
            <div class="step" data-step="2" id="step2-indicator"><div class="step-number">2</div><div class="step-label"><?php echo t('select_category'); ?></div></div>
            <div class="step" data-step="3" id="step3-indicator"><div class="step-number">3</div><div class="step-label"><?php echo t('select_items'); ?></div></div>
            <div class="step" data-step="4" id="step4-indicator"><div class="step-number">4</div><div class="step-label"><?php echo t('review_order'); ?></div></div>
        </div>
        ...
        <div id="step3" class="step-content">
            <div class="content-card">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
                    ...
                    <span id="selected-category-name" style="background: rgba(233,236,239,0.7); padding:5px 15px; border-radius:30px; font-size:0.85rem; font-weight:500;"></span>
                </div>
        ...
        <div id="step4" class="step-content">
            <div class="content-card">
                ...
                <div id="final-order-summary" style="margin: 20px 0;">
                    <div style="background:rgba(248,250,252,0.6); border-radius:28px; padding:20px;">
    ...
    <div id="session-orders-modal" class="quantity-modal">
        <div class="quantity-modal-content" style="max-width: 600px; max-height: 80vh; overflow-y: auto;">
            ...
            <div id="session-orders-list" style="text-align: left; margin-top: 15px;"><?php echo t('loading'); ?></div>
            <div class="modal-buttons" style="margin-top: 20px; display: flex; justify-content: space-between;">
                ...
                <button class="btn-sm btn-info" onclick="openParkingModal()"><?php echo t('parking_request'); ?></button>
            </div>
        </div>
    </div>
    ...
    <!-- Description Modal -->
    <div id="description-modal" class="quantity-modal">
        <div class="quantity-modal-content" style="max-width: 400px;">
            <h3 id="description-modal-title"><?php echo t('item_description'); ?></h3>
            <div id="description-modal-text" style="margin: 20px 0; text-align: left; font-size: 1rem; line-height: 1.5; max-height: 300px; overflow-y: auto;"></div>
            <div class="modal-buttons" style="margin-top: 20px;">
                <button class="btn-cancel" onclick="closeDescriptionModal()"><?php echo t('close'); ?></button>
            </div>
        </div>
    </div>
    <div class="floating-actions">
        <button class="floating-btn cancel" id="floating-cancel-btn" title="<?php echo t('cancel_order'); ?>">🗑️<span class="floating-badge" id="cancel-badge" style="display:none;">0</span></button>
        <button class="floating-btn send" id="floating-send-btn" title="<?php echo t('send_to_kitchen'); ?>">🚀<span class="floating-badge" id="order-badge" style="display:none;">0</span></button>
    </div>
    <div id="loading-overlay" class="loading-overlay"><div class="loading-spinner">⏳ <?php echo t('sending_order'); ?>...</div></div>
    <script>
    // ============================================
    // GLOBAL VARIABLES
    // ============================================
    ...
    let categories = <?php echo json_encode(array_values($categories)); ?>;
    let tables = <?php echo json_encode(array_values($tables)); ?>;
    let allItems = [];
    ...
    function openTableSession(tableId, capacity) {
        ...
        fetch('api/open_session.php', {
            ...
            body: JSON.stringify({ table_id: tableId, customer_count: parseInt(customerCount) })
        })
    ...
    function closeTableSession(tableId) {
        ...
        if (!printBill) {
            return;  // <-- this prevents closing the table
        }
        ...
        fetch('api/close_session.php', {
            ...
            body: JSON.stringify({ 
                table_id: tableId, 
                lang: currentLang,
                print_bill: true   // only called if user clicked OK
            })
        })
        .then(response => response.json())
        .then(data => {
    ...
    function showStep(step) {
        ...
        currentStep = step;
        if (step === 4) renderFinalOrder();
    }

    document.querySelectorAll('.step').forEach(step => {
        step.addEventListener('click', () => {
            var targetStep = parseInt(step.dataset.step);
            if (targetStep <= currentStep) showStep(targetStep);
        });
    });

    // Make entire table card clickable
    document.querySelectorAll('.table-card-select').forEach(card => {
        card.addEventListener('click', e => {
            ...
            var capacity = parseInt(card.dataset.capacity);
            if (sessionStatus === 'open') openTableForOrder(tableId, tableName);
            else openTableSession(tableId, capacity);
        });
    });
    ...
    function loadItems(categoryId, kitchenId, kitchenName) {
        ...
        fetch('ajax_get_items.php?category_id=' + categoryId + '&t=' + Date.now())
            .then(r => r.json())
            .then(items => { allItems = items; displayItems(allItems); });
    }
    ...
    function displayItems(items) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            html += '<div class="item-name-visual">' + item.name + ' <span class="item-info-btn" data-description="' + escapeHtml(item.description || '') + '">ℹ️</span></div>';
        ...
        // Attach description button listeners immediately after rendering
        attachDescriptionListeners();
        
        document.querySelectorAll('.item-card-visual').forEach(card => {
            card.addEventListener('click', () => {
                currentItem = {
                    id: parseInt(card.dataset.itemId),
                    name: card.dataset.itemName,
                    price: parseFloat(card.dataset.itemPrice),
                    kitchen_id: parseInt(card.dataset.itemKitchen),
                    kitchen_name: card.dataset.itemKitchenName
                };
    ...
    function filterItems() {
        ...
        var clearBtn = document.getElementById('search-clear-btn');
        if (searchTerm !== '') clearBtn.style.display = 'flex';
        else clearBtn.style.display = 'none';
        if (searchTerm === '') { displayItems(allItems); return; }
        var filteredItems = allItems.filter(item => item.name.toLowerCase().includes(searchTerm));
        displayItems(filteredItems);
        if (filteredItems.length === 0) {
            document.getElementById('items-grid-container').innerHTML = '<p style="grid-column:1/-1; text-align:center; padding:40px; color:#e74c3c;">🔍 <?php echo t('no_items_found'); ?></p>';
        }
    }
    ...
    document.addEventListener('keydown', e => {
        if (e.key === 'Escape') {
            var searchInput = document.getElementById('item-search');
            if (searchInput && document.activeElement === searchInput) clearSearch();
        }
    });
    ...
    document.getElementById('modal-qty-minus').addEventListener('click', () => {
        ...
        if (qty > 1) document.getElementById('modal-qty-value').textContent = qty - 1;
    });
    ...
    document.getElementById('modal-add').addEventListener('click', () => {
        ...
        for (var i = 0; i < currentOrder.length; i++) {
            if (currentOrder[i].id === currentItem.id) { existingIndex = i; break; }
        }
        if (existingIndex !== -1) {
            ...
            currentOrder.push({
                ...
                kitchen_id: currentItem.kitchen_id, kitchen_name: currentItem.kitchen_name,
                qty: qty, notes: notes, subtotal: currentItem.price * qty
            });
        }
        updateOrderSummary();
    ...
    function updateOrderSummary() {
        ...
        for (var i = 0; i < currentOrder.length; i++) {
            ...
            html += '<div><strong>' + item.qty + 'x ' + item.name + '</strong><br><small>🏠 ' + item.kitchen_name + '</small></div>';
    ...
    function updateFloatingButtons() {
        ...
        var itemCount = 0;
        for (var i = 0; i < currentOrder.length; i++) itemCount += currentOrder[i].qty;
    ...
    function renderFinalOrder() {
        ...
        if (currentOrder.length === 0) {
            container.innerHTML = '<div style="background:rgba(248,250,252,0.6); border-radius:28px; padding:20px;"><p style="color:#6c757d; text-align:center;"><?php echo t('no_order_to_review'); ?></p></div>';
            return;
        }
        var itemsHtml = '';
        var total = 0;
        for (var i = 0; i < currentOrder.length; i++) {
            ...
            if (item.notes) itemsHtml += '<br><small style="color:#f39c12;">📝 ' + item.notes + '</small>';
            itemsHtml += '<br><small>🏠 ' + item.kitchen_name + '</small></div>';
            itemsHtml += '<div>' + formatPrice(item.subtotal) + '</div></div>';
            total += item.subtotal;
        }
        container.innerHTML = '<div style="background:rgba(248,250,252,0.6); border-radius:28px; padding:20px;">' +
            ...
            '<span><?php echo t('total'); ?>:</span><span>' + formatPrice(total) + '</span></div></div>';
    }
    ...
    function sendOrder() {
        if (currentOrder.length === 0) { showToast('No items to send', 'error'); return; }
        if (!selectedTableId) { showToast('Please select a table first', 'error'); showStep(1); return; }
        var total = 0, itemCount = 0;
        for (var i = 0; i < currentOrder.length; i++) { total += currentOrder[i].subtotal; itemCount += currentOrder[i].qty; }
        if (confirm('Send order to kitchen?\n\nTable: ' + selectedTableName + '\n' + itemCount + ' items\n' + formatPrice(total))) {
            var orderData = { table_id: selectedTableId, table_number: selectedTableName, items: [] };
            for (var i = 0; i < currentOrder.length; i++) {
                orderData.items.push({
                    id: currentOrder[i].id, name: currentOrder[i].name, quantity: currentOrder[i].qty,
                    notes: currentOrder[i].notes, kitchen_id: currentOrder[i].kitchen_id, price: currentOrder[i].price
                });
            }
            showLoading();
            ...
            .then(data => {
                hideLoading();
                if (data.success) { showToast('✅ Order sent to kitchen!', 'success'); resetOrder(); }
                else { showToast('❌ ' + data.message, 'error'); }
            })
            .catch(error => { hideLoading(); console.error('Error:', error); showToast('❌ Failed to send order', 'error'); });
        }
    }
    ...
    function resetOrder() {
        ...
        renderFinalOrder();
        updateFloatingButtons();
        document.querySelectorAll('.table-card-select').forEach(c => c.classList.remove('selected'));
        document.querySelectorAll('.category-card').forEach(c => c.classList.remove('selected'));
        showStep(1);
    }
    ...
    document.getElementById('back-step4').addEventListener('click', () => showStep(3));
    ...
    document.getElementById('quantity-modal').addEventListener('click', e => { if (e.target === this) this.style.display = 'none'; });
    ...
    function showSessionOrders(tableId, tableName) {
        fetch('api/get_session_by_table.php?table_id=' + tableId)
            ...
            .catch(error => { console.error('Error:', error); showToast('<?php echo t('error'); ?>', 'error'); });
    }
    ...
    function loadSessionOrdersForModal(sessionId) {
        ...
        fetch('api/get_session_orders.php?session_id=' + sessionId)
            ...
            .then(data => {
                ...
                if (!data.orders || data.orders.length === 0) { container.innerHTML = '<div><?php echo t('no_orders'); ?></div>'; return; }
                let html = '';
                data.orders.forEach(order => {
                    ...
                    (order.items || []).forEach(item => {
                        ...
                        if (!isCancelled) html += '<button class="btn-sm btn-danger" onclick="cancelOrderItem(' + item.id + ')"><?php echo t('cancel_item'); ?></button>';
                        else html += '<span class="cancelled-badge"><?php echo t('cancelled'); ?></span>';
                        html += '</td></tr>';
                    });
                    html += '</table></div>';
                });
            ...
            .catch(error => { console.error('Error:', error); container.innerHTML = '<div style="color: red;"><?php echo t('error_loading'); ?></div>'; });
    }
    ...
    function cancelOrderItem(orderItemId) {
        ...
        .then(data => {
            hideLoading();
            if (data.success) { showToast('✅ ' + data.message, 'success'); if (currentSessionId) loadSessionOrdersForModal(currentSessionId); }
            else { showToast('❌ ' + data.message, 'error'); }
        })
        .catch(error => { hideLoading(); console.error('Error:', error); showToast('<?php echo t('error'); ?>', 'error'); });
    }

    function closeSessionOrdersModal() { document.getElementById('session-orders-modal').style.display = 'none'; }

    function printControllerTicket() {
        if (!currentSessionId) { showToast('<?php echo t('error'); ?>: No session selected', 'error'); return; }
        showLoading();
        ...
        .then(response => response.json())
        .then(data => { hideLoading(); if (data.success) showToast('✅ ' + data.message, 'success'); else showToast('❌ ' + data.message, 'error'); })
        .catch(error => { hideLoading(); console.error('Error:', error); showToast('<?php echo t('error'); ?>', 'error'); });
    }

    let currentParkingSessionId = null;
    function openParkingModal() {
        if (currentSessionId) { currentParkingSessionId = currentSessionId; document.getElementById('parking-modal').style.display = 'flex'; document.getElementById('parking-lot-numbers').value = ''; }
        else showToast('<?php echo t('error'); ?>: No session selected', 'error');
    }
    ...
    function sendParkingRequest() {
        ...
        if (!lotNumbers.trim()) { showToast('<?php echo t('error'); ?>: Please enter parking lot numbers', 'error'); return; }
        showLoading();
        fetch('api/send_parking_request.php', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ session_id: currentParkingSessionId, lot_numbers: lotNumbers })
        })
        .then(response => response.json())
        .then(data => {
            hideLoading();
            if (data.success) { showToast('✅ ' + data.message, 'success'); closeParkingModal(); }
            else showToast('❌ ' + data.message, 'error');
        })
        .catch(error => { hideLoading(); console.error('Error:', error); showToast('<?php echo t('error'); ?>', 'error'); });
    }
    ...
    function showDescription(description) {
        ...
        const textDiv = document.getElementById('description-modal-text');
        textDiv.innerHTML = description ? escapeHtml(description) : '<?php echo t('no_description'); ?>';
    ...
    function attachDescriptionListeners() {
        document.querySelectorAll('.item-info-btn').forEach(btn => {
            btn.removeEventListener('click', descriptionClickHandler);
            btn.addEventListener('click', descriptionClickHandler);
        });
    }

...
try {
    ...
    foreach ($data['items'] as $item) {
        ...
        // Get item name and print_on_controller flag
        $nameQuery = "SELECT name, print_on_controller FROM menu_items WHERE id = $menu_item_id";
        $nameResult = $db->query($nameQuery);
        $itemInfo = $nameResult->fetch();
        ...
        $items_by_kitchen[$kitchen_id][] = [
            ...
            'notes' => $item['notes'] ?? '',
            'price' => $price,
            'print_on_controller' => $itemInfo['print_on_controller']
        ];
    }
    ...
    // ========== ORDER TIME ADDED ==========
    // Get the exact order creation timestamp
    $orderTimeStmt = $db->prepare("SELECT created_at FROM orders WHERE id = ?");
    $orderTimeStmt->execute([$order_id]);
    $order_created_at = $orderTimeStmt->fetchColumn();
    // Build cross‑kitchen summary for each kitchen
    $cross_summaries = [];
    foreach ($items_by_kitchen as $kitchen_id => $kitchen_items) {
        $other_items = [];
        foreach ($items_by_kitchen as $other_id => $other_items_list) {
            if ($other_id == $kitchen_id) continue;
            foreach ($other_items_list as $item) {
                $other_items[] = $item['quantity'] . 'x ' . $item['name'];
            }
        }
        if (!empty($other_items)) {
            $cross_summaries[$kitchen_id] = "--- " . t('other_kitchens') . " ---\n" . implode("\n", $other_items);
        } else {
            $cross_summaries[$kitchen_id] = '';
        }
    }
    // ====================================
    ...
    try {
        ...
        foreach ($items_by_kitchen as $kitchen_id => $kitchenItems) {
            ...
            $printer = [
                ...
                'fallback_name' => $kitchen['fallback_printer_name'] ?? '',
                'ip' => $kitchen['printer_ip'] ?? '',
                'port' => $kitchen['printer_port'] ?? 9100
            ];
            
            // PASS THE ORDER TIME
            //$content = formatKitchenPrint($db, $kitchen['name'], $order_number, $data['table_number'], $kitchenItems, false, 56, $waiterName, $order_created_at);
            $content = formatKitchenPrint($db, $kitchen['name'], $order_number, $data['table_number'], $kitchenItems, false, 56, $waiterName, $order_created_at, $cross_summaries[$kitchen_id]);
        ...
        $tableCheck = $db->prepare("SELECT is_reserved FROM restaurant_tables WHERE id = ?");
        $tableCheck->execute([$table_id]);
        $isReserved = $tableCheck->fetchColumn();

        $customerStmt = $db->prepare("SELECT customer_name FROM restaurant_tables WHERE id = ?");
        $customerStmt->execute([$table_id]);
        $customer_name = $customerStmt->fetchColumn();

        if ($accountantSettings['enabled'] && !$isReserved) {
            $accountantPrinter = [
            ...
            $content = formatAccountantPrint($db, $order_number, $data['table_number'], $accountantItems, $total_amount, $waiterName, $sessionNumber, $customer_name);
            ...
            file_put_contents($debug_file, date('Y-m-d H:i:s') . " Accountant print skipped for reserved table $table_id\n", FILE_APPEND);
        }

        // 3. Print controller ticket (if enabled) – only if there are items with print_on_controller = 1
        $controllerSettings = getControllerPrinterSettings($db);
        if ($controllerSettings['enabled'] && !empty($controllerSettings['name'])) {
            $hasControllerItems = false;
            foreach ($items_by_kitchen as $kitchenItems) {
                foreach ($kitchenItems as $item) {
                    if (isset($item['print_on_controller']) && $item['print_on_controller'] == 1) {
                        $hasControllerItems = true;
                        break 2;
                    }
                }
            }
            if ($hasControllerItems) {
                // PASS THE ORDER TIME
                $controllerContent = formatControllerTicket($db, $items_by_kitchen, $order_number, $data['table_number'], $sessionNumber ?? '', $waiterName, $order_created_at);
                if (!empty($controllerContent)) {
                    $controllerPrinter = [
                        'type' => 'windows',
                        'name' => $controllerSettings['name']
                    ];
                    sendPrintJob($controllerPrinter, $controllerContent);
                }
            } else {
                file_put_contents($debug_file, date('Y-m-d H:i:s') . " No controller items, skipping controller ticket\n", FILE_APPEND);
            }
        }

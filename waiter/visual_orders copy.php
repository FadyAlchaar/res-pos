...
<body>
    <div class="wizard-container">
        <div class="header">
            <div class="logo">
                <h1>🍽️ <?php echo t('waiter_terminal'); ?> <span style="font-size:0.7rem; background:#e9ecef; padding:2px 8px; border-radius:20px;">Visual Mode</span></h1>
            </div>
        ...
        <div id="step1" class="step-content active">
            <div class="content-card">
                ...
                <p style="color:#6c757d; margin-bottom:20px;"><?php echo t('choose_table_to_start'); ?></p>
        ...
        <div id="step2" class="step-content">
            <div class="content-card">
                ...
                <p style="color:#6c757d; margin-bottom:20px;"><?php echo t('choose_food_category'); ?></p>
    ...
    function closeTableSession(tableId) {
        if (!confirm('Close this table? This will finalize the bill and free the table.')) return;
        ...
        .then(data => {
            hideLoading();
            if (data.success) {
                showToast('💰 Table closed. Total: $' + parseFloat(data.total).toFixed(2), 'success');
                location.reload();
            } else {
                showToast('❌ Error: ' + data.message, 'error');
            }
    ...
    // ============================================
        ...
        document.querySelectorAll('.table-card-select').forEach(function(card) {
            card.addEventListener('click', function(e) {
                // If the click originated from a button inside, ignore because button already handles it
                ...
                if (sessionStatus === 'open') {
                    ...
                    // Closed session -> open table (prompt for customer count)
    ...
    function displayItems(items) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            html += '<div class="item-price-visual">$' + parseFloat(item.price).toFixed(2) + '</div>';
    ...
    function updateOrderSummary() {
        ...
        for (var i = 0; i < currentOrder.length; i++) {
            ...
            html += '</div>';
            html += '<div style="color:#27ae60; font-weight:bold;">$' + item.subtotal.toFixed(2) + '</div>';
            html += '</div>';
            total += item.subtotal;
        }
    ...
    function renderFinalOrder() {
        ...
        for (var i = 0; i < currentOrder.length; i++) {
            ...
            itemsHtml += '</div>';
            itemsHtml += '<div>$' + item.subtotal.toFixed(2) + '</div>';
            itemsHtml += '</div>';
            total += item.subtotal;
        }
        ...
        if (confirm('Send order to kitchen?\n\nTable: ' + selectedTableName + '\n' + itemCount + ' items\n$' + total.toFixed(2))) {
            ...
            for (var i = 0; i < currentOrder.length; i++) {
                orderData.items.push({
                    ...
                    name: currentOrder[i].name,        // <-- ADD THIS LINE
